...
public class SensorModule implements SensorEventListener{
    ...
    public void start(long start_time, FileModule file_in){
        flag_is_sensor_running = true; // 현재 센서데이터 측정 중이다
        sensor_measurement_start_time = start_time;
        file = file_in;

        sm.registerListener(this, s1, sensor_sampling_interval_ms * 1000);
        sm.registerListener(this, s2, sensor_sampling_interval_ms * 1000);
        sm.registerListener(this, s3, sensor_sampling_interval_ms * 1000);
        sm.registerListener(this, s4, sensor_sampling_interval_ms * 1000);
        sm.registerListener(this, s5, sensor_sampling_interval_ms * 1000);
        sm.registerListener(this, s6, sensor_sampling_interval_ms * 1000);
        sm.registerListener(this, s7, sensor_sampling_interval_ms * 1000);
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        if (sensorEvent.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            Matrix.multiplyMV(accW,0,rot_mat_opengl,0,accL,0);//뚱뚱한 3by3 매트릭스 곱하기 벡터를 수행하겠다
       ...
       str += String.format("Pressure: %2.3f hPa\n", pres);
        ...
        str += String.format("AccW x : %2.3f, y : 2.3%f, z : 2.3%f\n", accW[0],accW[1],accW[2]);
        current_state = str;
        //tv.setText(str);
    }
